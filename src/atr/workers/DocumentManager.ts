...
export class DocumentManager {
	...
	private removeFileExtensionRegex: RegExp = /\.[^.]+$/;
	...
	public async addDocument(currentFile: QueueFile, hash: string, finalPath: string, size: number, documentType: FileDocumentType): Promise<void> {
		let requestedName: string = currentFile.DesiredName;
		console.log('DESNAME', requestedName);

		if (requestedName !== undefined) {
			console.log('New doc of type', documentType, 'called', requestedName, 'with size', size, 'MB, hash of', hash, 'from', currentFile.Path);
		} else {
			console.log('New doc of type', documentType, 'with a size of', size, 'MB and a hash of', hash, 'from', currentFile.Path);
		...
		await this.databaseManager.addEntryToFileTable(hash, finalPath, documentType, Date.now(), requestedName, currentFile.OriginalName);
	}
}
